...
bool perf_test()
{
    ...
    const float* stacked_cqt_data = feature_calculator.computeFeatures(audio.data(), audio.size(), num_out_frames);

    auto stop_time_onnx = std::chrono::high_resolution_clock::now();

    std::vector<std::vector<float>> contours(num_out_frames, std::vector<float>(NUM_FREQ_IN));
    std::vector<std::vector<float>> notes(num_out_frames, std::vector<float>(NUM_FREQ_OUT));
    std::vector<std::vector<float>> onsets(num_out_frames, std::vector<float>(NUM_FREQ_OUT));

    for (size_t i = 0; i < num_out_frames; i++) {
        cnn.frameInference(stacked_cqt_data + i * NUM_HARMONICS * NUM_FREQ_IN, contours[i], notes[i], onsets[i]);
    }
