...
template <typename T>
static std::vector<std::vector<T>> convert_1d_to_2d(std::vector<T> flattened_vec, int rows, int cols)
{
    if (rows < 0) {
        if (cols < 0) {
            throw std::runtime_error("convert_1d_to_2d: either rows or cols have to be specified (i.e. positive)");
        }
        rows = safe_divide(flattened_vec.size(), cols);
    } else if (cols < 0) {
        cols = safe_divide(flattened_vec.size(), rows);
    } else if (flattened_vec.size() != rows * cols) {
        throw std::runtime_error(std::string("convert_1d_to_2d: invalid size of vector") + std::to_string(flattened_vec.size()) + std::string(". Expected ") + std::to_string(rows * cols));
    }
